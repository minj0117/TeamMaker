...
import com.kh.prj.common.authSVC;
...
public class TeamboardController {
	...
	public String boardList(@RequestParam("tno") int tno, TeammemberVO vo, Model model, PagingCriteria cri,
			HttpSession session) {
		String id = (String) session.getAttribute("id");
		vo.setUserid(id);
		vo.setTno(tno);
		int check = teamSVC.checkmember(vo);
		String owner = teamSVC.checkowner(tno);
		if (check == 1 || owner.equals(id)) {
			cri.setTno(tno);
	...
	@GetMapping("/talkList")
	public String talkList(@RequestParam("tno") int tno, Model model, PagingCriteria cri, HttpSession session) {
		String id = (String) session.getAttribute("id");
		int result = auth.auth(id, tno);
		String check = auth.ownercheck(tno);
		if (result == 1 || check.equals(id)) {
			cri.setTno(tno);
	...
	@GetMapping("/fileList")
	public String fileList(@RequestParam("tno") int tno, Model model, PagingCriteria cri, HttpSession session) {
		String id = (String) session.getAttribute("id");
		int result = auth.auth(id, tno);
		String check = auth.ownercheck(tno);
		if (result == 1 || check.equals(id)) {
	...
	public String boardView(@RequestParam("bno") int bno, @RequestParam("tno") int tno, Model model,
			HttpSession session) {
		String id = (String) session.getAttribute("id");
		int result = auth.auth(id, tno);
		String check = auth.ownercheck(tno);
	...
	public String fileuploadForm(@RequestParam("tno") int tno, Model model, HttpSession session) {
		String id = (String) session.getAttribute("id");
		int result = auth.auth(id, tno);
		String check = auth.ownercheck(tno);
		if (result == 1 || check.equals(id)) {
	 ...
	 */
	@GetMapping("/fileView")
	public String fileView(@RequestParam("bno") int bno, @RequestParam("tno") int tno, Model model,
			HttpSession session) {
		String id = (String) session.getAttribute("id");
		int result = auth.auth(id, tno);
		String check = auth.ownercheck(tno);
